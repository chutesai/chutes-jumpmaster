...
class RecoverImageBuildTests(unittest.TestCase):
    ...
    def test_parse_image_ref_allows_slashes_in_image_name_and_tag(self):
        ref = recover_image_build.parse_image_ref("alice/group/model:release/cuda")
        self.assertEqual(ref.username, "alice")
        self.assertEqual(ref.name, "group/model")
        self.assertEqual(ref.tag, "release/cuda")
    ...
    def test_extract_original_dockerfile_steps_explains_missing_retained_log(self):
        with self.assertRaises(ValueError) as exc:
            recover_image_build.extract_original_dockerfile_steps("built and pushed")
        self.assertIn("no retained Dockerfile build log", str(exc.exception))

    def test_parse_env_instruction_preserves_quoted_spaces(self):
        self.assertEqual(
            recover_image_build.parse_env_instruction('ENV A="two words" B=plain'),
            [("A", '"two words"'), ("B", "plain")],
        )
        self.assertEqual(
            recover_image_build.parse_env_instruction('ENV A two words'),
            [("A", "two words")],
        )
    ...
    def test_retry_exhausted_message_for_rate_limit(self):
        message = recover_image_build.retry_exhausted_message("op", 429, "too much", 8)
        self.assertIn("HTTP 429 after 8 retries", message)
        self.assertIn("--scan-concurrency", message)

    def test_recover_passes_retry_settings_to_fetchers(self):
        args = argparse.Namespace(
            image="sglang",
            engine="auto",
            model_name="MODEL",
        ...
        with mock.patch("recover_image_build.fetch_images", new_callable=mock.AsyncMock) as fetch_images:
            with mock.patch(
                "recover_image_build.fetch_image_logs", new_callable=mock.AsyncMock
            ) as fetch_logs:
                fetch_images.return_value = [image]
                fetch_logs.return_value = "STEP 1/1: FROM base\n"
                with redirect_stdout(io.StringIO()):
                    status = asyncio.run(recover_image_build.recover(args))
        self.assertEqual(status, 0)
        fetch_images.assert_awaited_once_with(
            "sglang", max_retries=9, retry_base_delay=3.0
        )
        fetch_logs.assert_awaited_once_with(
            "img-1", max_retries=9, retry_base_delay=3.0
        )

    def test_resolve_image_uses_direct_fetch_for_exact_owner_ref(self):
        ref = recover_image_build.parse_image_ref("chutes/sglang:nightly-2026051700")
        image = self.sample_image()
        with mock.patch(
            "recover_image_build.fetch_image_by_ref", new_callable=mock.AsyncMock
        ) as fetch_direct:
            with mock.patch("recover_image_build.fetch_images", new_callable=mock.AsyncMock) as fetch_list:
                fetch_direct.return_value = image
                resolved = asyncio.run(
                    recover_image_build.resolve_image(
                        ref, max_retries=7, retry_base_delay=2.5
                    )
                )
        self.assertIs(resolved, image)
        fetch_direct.assert_awaited_once_with(
            "chutes/sglang:nightly-2026051700",
            max_retries=7,
